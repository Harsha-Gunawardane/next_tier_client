...
const SearchBar = (props) => {
    ...
    return (
        <FormControl id="search" h="40px" width={"300px"} {...rest}>
            <InputGroup borderRadius={"10px"}>
                <InputLeftElement pointerEvents='none' color={"gray.400"}>
                    <HiSearch size="18px" />
                </InputLeftElement>
                <Input type='tel' placeholder='Search' variant={"filled"} value={search} onChange={(e) => { setSearch(e.target.value) }} borderRadius={"10px"} />
                <InputRightElement pointerEvents={"all"} display={search ? "block" : "none"} zIndex={90}>
